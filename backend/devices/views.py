...
def action_device_norm(type,data):
    ...
    if type == "create" or type == "update":
        template_data={
            ...
            "io_rkbs": {
                "min_a": data["io_rkbs_min"],
                "max_a": data["io_rkbs_max"]
            },
            "io_wkbs": {
                "min_a": data["io_wkbs_min"],
                "max_a": data["io_wkbs_max"]
            }
        }
        if r.get("device_norm"):
            data_device_norm=json.loads(r.get("device_norm").decode())
        else: data_device_norm={}
    ...
    def get(self, request, pk, format=None):
        ...
        if r.get("device_check_have_data"):
            ...
            if device_check_have_data.get(f'{device_id}'):
                ...
                device_check_have_data[f'{device_id}']={f'{device_id}':{"status":"GOTACTU","timestamp":time_check}}
            r.set("device_check_have_data",json.dumps(device_check_have_data))
        else :
            device_check_have_data = {f'{device_id}':{"status":"GOTACTU","timestamp":time_check}}
            r.set("device_check_have_data",json.dumps(device_check_have_data))
                
        tasks.publish_message(
            {'type': 'kill', 'id': serializer.data['id'], 'ip_agent': serializer.data['ip'],
             'protocol': serializer.data['protocol'],
             'ip_django': os.getenv("LAN_IP"),
             "username": serializer.data['username'], "password": serializer.data['password'],'OS': serializer.data['os'] or 'unknown', 'port': serializer.data['port'],'agent_name':serializer.data['agent_name'],'hwagent': serializer.data['hwagent'],'device_type':serializer.data["device_type"] })
...
class install_agent(APIView):
    ...
    ip_sock_serv = openapi.Parameter(
        'ip_sock_serv', openapi.IN_QUERY, description='ip_sock_serv',  type=openapi.TYPE_STRING)
    ...
    Devices_Serializer = openapi.Response(
        "DevicesSerializer", DevicesSerializer)
    ...
    @swagger_auto_schema(method="get", manual_parameters=[ip_agent, ip_django, ip_sock_serv, username, password, avatar
                                                        #   , os
                                                          ], responses={200: Devices_Serializer})
    @api_view(["GET"])
    def get(self, request, format=None):
        tasks.publish_message(
            {'type': 'install', 'ipagent': request.query_params['ipagent'],
             'ip_django': request.query_params['ipdjango'], "ipsockserv": request.query_params['ipsockserv'],
             "username": request.query_params['username'], "password": request.query_params['password'],"os": request.query_params['os'], "port": request.query_params['port']} )
...
class DevicesViewSet(
    viewsets.ModelViewSet, generics.CreateAPIView
):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        if r.get(f"cpu_ram_deivce_{data['id']}"):
            data_device_redis = json.loads(r.get(f"cpu_ram_deivce_{data['id']}").decode())
    ...
    def update_rulebase_hw(self, pk):
        ...
        if current_device.hwagent:
            tasks.publish_message(
                {'type': 'update rulebase hw', 'id': current_device.id, 'ip_agent':current_device.ip,
                'protocol': current_device.protocol,
                'ip_django': os.getenv("LAN_IP"),
                'ip_sock_serv': os.getenv("LAN_SOCKET_SERVER_IP") if not current_device.from_internet else os.getenv("PUBLIC_SOCKET_SERVER"),
                'ip_tftp_serv': os.getenv("LAN_SOCKET_SERVER_IP") if not current_device.from_internet else os.getenv("PUBLIC_TFTP_SERVER"),
                "username": current_device.username, "password": current_device.password,
    ...
    def reboot(self, request):
        try:
            ...
            tasks.publish_message(
                {'type': 'reboot', 'id': current_device.id, 'ip_agent': current_device.ip,
                 'protocol': current_device.protocol,
                 'ip_django': os.getenv("LAN_IP"),
                 "username": current_device.username, "password": current_device.password})
    ...
    def isolate(self, request):
        try:
            ...
            tasks.publish_message(
                {'type': 'isolate', 'id': current_device.id, 'ip_agent': current_device.ip,
                 'protocol': current_device.protocol,
                 'ip_django': os.getenv("LAN_IP"),
                 "username": current_device.username, "password": current_device.password})
            return Response({"status": 200})
    ...
    @swagger_auto_schema(
        method='patch',
        request_body= InstallAgentSerializer
    ...
    def install_list_agent(self, request):
        try:
            ...
            for dt in data['id']:
                ...
                'ip_django': os.getenv("LAN_IP"),
                'ip_sock_serv': os.getenv("LAN_SOCKET_SERVER_IP") if not currentDevice.from_internet else os.getenv("PUBLIC_SOCKET_SERVER"),
                'ip_tftp_serv': os.getenv("LAN_SOCKET_SERVER_IP") if not currentDevice.from_internet else os.getenv("PUBLIC_TFTP_SERVER"),
                "username": currentDevice.username, "password": currentDevice.password,
                'OS': currentDevice.os,
    ...
    def kill_list_agent(self, request):
        try:
            ...
            for dt in data['id']:
                ...
                id=dt).order_by('-created').first()
                tasks.publish_message(
                {'type': 'kill', 'id': currentDevice.id, 'ip_agent': currentDevice.ip,
                'protocol': currentDevice.protocol,
                'ip_django': os.getenv("LAN_IP"),
                "username": currentDevice.username, 
                "password": currentDevice.password,'OS':  currentDevice.os or 'unknown', 
                'port': currentDevice.port,
                'hwagent': currentDevice.hwagent
                })
            return Response({"message": "handling.."})
                ...
                for dt in data:
                    ...
                    for dt_white in whitelist:
                        if ip.ip == dt_white.ip:
                            ...
                            if stt:
                                messss =dt_white.ip + " đã có trong WhiteList không thể thêm vào BlackList"
                                stt = False
                            else:
                ...
                for dt in data:
                    ...
                    for dt_black in blacklist:
                        if ip.ip == dt_black.ip:
                            ...
                            if stt:
                                messss =dt_black.ip + " có trong BlackList và đã bị xóa"
                                stt = False
                            else:
...
class ChangePwdView(generics.GenericAPIView):
    ...
    def patch(self, request, pk, format=None):
        ...
        createFileObject = CreateInfoFile(
            xxx_data['password'], telnet_root_dir)
        createFileObject.do_create_file()
...
class AlertsViewSet(viewsets.ModelViewSet):
    ...
    def kill_process(self, request):
        try:
            currentDevice = Devices.objects.filter(
                id=request.query_params['device_id']).order_by('-created').first()

            tasks.publish_message(
                {'type': 'kill_process', 'id': currentDevice.id, 'ip_agent': currentDevice.ip,
                 'ip_django': os.getenv("LAN_IP"), "pid": request.query_params['pid'], "alert_id": request.query_params["alert_id"],
                 "username": currentDevice.username, "password": currentDevice.password, 'protocol': currentDevice.protocol
                 })
            return Response({"message": "handling.."})
    ...
    def exportXLSX(self,request):
        try:
            ...
            header = ["STT", "ID" ,"IP", "Timestamp", "Hash","PID", "Message", "Type" ,"Rule_base", "Log", "Status", "Device"]
            col_stt = 1
            ...
            for dt_res  in result_id:
                    ...
                    sheet2.write(rol[3]+ str(col_stt) , dt_res.timestamp.strftime('%Y-%m-%d %H:%M:%S') )
...
class ProcessHashViewSet(viewsets.ModelViewSet):
    ...
    def check_exist(self, request):
        try:
            result = ProcessHash.objects.get(
                sha1=request.query_params['sha1'])
...
class ProcessListViewSet(viewsets.ModelViewSet):
    ...
    def GetProcessListByDeviceId(self, request):
        try:
            ...
            if searchKey != None:
                json_processlist = [x for x in json_processlist if ((str(x.get('pid', None)) == searchKey) or (searchKey in x.get('sha1')))]
    ...
    def AddProcessListWithIP(self, request):
        try:
            ...
            serializer = ProcessListSerializer(
                data={**{"process_list": xxx_data["process_list"]}, **{"device_id": current_device.id}})
            if serializer.is_valid(raise_exception=True):
                serializer.save()
                return Response(serializer.data, status=status.HTTP_201_CREATED)
...
class SyscallListViewSet(viewsets.ModelViewSet):
    ...
    def GetSyscallListByDeviceID(self, request):
        try:
            result = SyscallList.objects.filter(
                device_id=request.query_params['device_id']).order_by('-created')
    ...
    def TraceSyscall(self, request):
        try:
            ...
            if not currentDevice.agentInstalled:
                return Response(data={'message': 'Thiết bị chưa được cài tác tử'}, status=status.HTTP_400_BAD_REQUEST)
            currentDevice.tracing_syscall = request.query_params['pid']
            currentDevice.save()
            # TODO call syscall tracing func to agent
            tasks.publish_message(
                {'type': 'stop_trace_syscall'})
            tasks.publish_message(
                {'type': 'install_syscall', 'id': currentDevice.id, 'ip_agent': currentDevice.ip,
                 'ip_django': os.getenv("LAN_IP"), "pid": request.query_params['pid'], "remote_port": currentDevice.remote_port 
                 })
    ...
    def TraceNetwork(self, request):
        ...
        try:
            ...
            # Nếu thiết bị chưa được cài agent thì hủy
            if not currentDevice.agentInstalled:
                return Response(data={'message': 'Thiết bị chưa được cài tác tử'}, status=status.HTTP_200_OK)
            ...
            tasks.publish_message(
                {'type': 'stop_trace_pcap','id': currentDevice.id, 'ip_agent': currentDevice.ip})
            tasks.publish_message(
                {'type': 'trace_pcap', 'id': currentDevice.id, 'ip_agent': currentDevice.ip, 
                 'ip_django': os.getenv("LAN_IP"), 'remote_port': currentDevice.remote_port
                 })
            return Response(DevicesSerializer(currentDevice).data)
    ...
    def AddInterityCheckWithIP(self, request):
        try:
            ...
            serializer = IntegrityCheckSerializer(
                data={**{"message": xxx_data["message"]}, **{"device_id": current_device.id}})
            if serializer.is_valid(raise_exception=True):
                serializer.save()
                return Response(serializer.data, status=status.HTTP_201_CREATED)
        except Exception as e:
    ...
    def check_version(self, request):
        try: 
            ...
            queryset = ModelML.objects.filter(
                ...
                ).order_by('-timestamp', 'version').count() 
            ...
            if queryset == 0: 
                ...
                data['results'][0]['file'] = re.sub(center_server_url + "media/","",data['results'][0]['file'] )
...
class IpsView(
    ...
    def add_dst_ip(self, request):
        try:
            ...
            for ip_track in xxx_data['ip_tracking']:
                ...
                if check == 0:        
                    ips_tracking_data = IpsTrackingSerializer(data = {
                        **{"device" : xxx_data['device']},
                        **{"ip_tracking" : ip_track},
                        **{"count" : 1}
                    })
    ...
    def list_ip_tracking(self, request):
        try:
            result = IpsTracking.objects.filter(device = request.query_params['device_id']).order_by("-count")
            page_size = request.query_params['page_size']
            page = request.query_params['current']

            if request.query_params.get('ip_tracking', None) != None:
                result = IpsTracking.objects.filter(ip_tracking = request.query_params['ip_tracking'])
    ...
    @swagger_auto_schema(
        method='patch',
        request_body = AddCountIpsTrackingSerializer
    )
    @action(detail=False, methods=['patch'])
...
class OneSignalView(viewsets.ModelViewSet):
    ...
    def find_device_id(self, request):
        try:
            result = OneSignal.objects.get(onesignal_id = request.query_params.get('onesignal_id', None),
    ...
    def delete_device(self, request):
        try:
            result = OneSignal.objects.filter(onesignal_id = request.query_params.get('onesignal_id', None),
                                            username = request.query_params.get('username', None)
                                            ).first()
            result.delete()
    ...
    def rulebase_info(self, request):
        if request.query_params.get('rules', None) == 'firewall':
            output = RuleBase.objects.filter(
                    rules='firewall').order_by('-timestamp').first()
        else:
            output = RuleBase.objects.filter(
...
class DeviceIpConnectHWagentView(viewsets.ModelViewSet):
    ...
    def get_ipdevice(self, request):
        try:
            ...
            result = DeviceIpConnectHWagent.objects.filter(device = device.id, network = False)
            data =[]
    ...
    def get_ip_in_device(self, request):
        try:
            ...
            if request.query_params.get('connect',None) == 'D':
                result = DeviceIpConnectHWagent.objects.filter(device = device.id, connect = 'D')
            else:
    ...
    def update_device_network_hwagent(self, request):
        try:
            ...
            for dt in xxx_data['id']:
                ...
                result.save()
            current_device = Devices.objects.filter(
                id=request.query_params.get("device_id", None)).first()
            if current_device.hwagent:
                tasks.publish_message(
                    {'type': 'update rulebase hw', 'id': current_device.id, 'ip_agent':current_device.ip,
                    'protocol': current_device.protocol,
...
class ThresholdView(APIView):
    def get(self, request):
        try:
            ...
            active_distributed_receive = get_env("ACTIVE_DISTRIBUTED_RECEIVE").lower() == 'true'
            return Response({
    ...
    def put(self, request):
        try:
            ...
            update_env("ACTIVE_DISTRIBUTED_RECEIVE", str(data.get("active_distributed_receive", True)))
            ...
            for i in range(len(arr)):
                ...
                if (arr[i].find('LATENCY_THRESHOLD') != -1):
                    arr[i] = 'LATENCY_THRESHOLD=' + str(data.get("latency_threshold", 10))
                if (arr[i].find('ACTIVE_DISTRIBUTED_RECEIVE') != -1):
                    arr[i] = 'ACTIVE_DISTRIBUTED_RECEIVE=' + str(data.get("active_distributed_receive", True))
                elif (arr[i].find('ACTIVE_DISTRIBUTED') != -1):
                    arr[i] = 'ACTIVE_DISTRIBUTED=' + str(data.get("active_distributed", True))
            res = ""
            for i in range(len(arr)):
                res += (arr[i])
                if i < len(arr) - 1:
...
class AgentClearFileSFTPView(APIView):
    def get(self, request):
        try:
            return Response(int(r_agent.get("AUTO_CLEAR_SFTP_STORAGE_MINUTES")), status=status.HTTP_200_OK)
        except Exception as err:
            return Response({
                "message": str(err)
            }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class AgentCheckOverloadView(APIView):
    def get(self, request):
        try:
            return Response(int(r_agent.get("AUTO_CHECK_OVERLOAD_SECONDS")), status=status.HTTP_200_OK)
        except Exception as err:
            return Response({
                "message": str(err)
            }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class DeviceNormViewSet(viewsets.ModelViewSet):
    ...
    def get_by_device_id(self, request, *args, **kwargs):
        try:
            ...
            if res != None:
                return Response(data=DeviceNormSerializer(res).data, status=status.HTTP_200_OK)
            else:
                ...
                if serializer_device_norm.is_valid():
                    ...
                    action_device_norm("create",serializer_device_norm.data)
                    action_check_device_norm(serializer_device_norm.data)
                return Response(serializer_device_norm.data, status=status.HTTP_201_CREATED)
        except Exception as e:
            logging.error(e)
            return Response(data={'status': str(e)}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def update_by_device_id(self, request, *args, **kwargs):
        try:
            ...
            if instance != None:
                ...
                return Response("DeviceNorm Not found", status=status.HTTP_404_NOT_FOUND)
        except Exception as e:
            logging.error(e)
            return Response(data={'status': str(e)}, status=status.HTTP_400_BAD_REQUEST)
...
class GetPcapJsonFromElasticsearch(viewsets.ModelViewSet):
    ...
    def get_data_pcap(self, request):
        try:
            ...
            if int(page_size) < 1 : page_size = 1
            es = Elasticsearch(os.environ.get("ELASTIC_HOST"))
            resp = es.search(index="pcap_save", query={"match_all": {}}, size= page_size, from_= int(page_size)*(int(page)-1))
...
class GetFileFromPcapWithTime(APIView):
    ...
    def post(self, request):
        try:
            ...
            for data in datas:
                if data[:-1]>f'{key_name_file}{time_start}.pcap' and data[:-1]<f'{key_name_file}{time_end}.pcap':
